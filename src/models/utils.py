...
class FPM(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden_states = self.LayerNorm(hidden_states + x)  # Applying layer normalization
...
class BehaviorSpecificPFF(nn.Module):
    ...
    def __init__(self, d_model, d_ff, n_b, bpff=False, dropout=0.1):
        ...
        self.bpff = bpff  # 是否使用行为特定的PFF
        if bpff and n_b > 1:  # 如果使用行为特定的PFF并且行为数量大于1
            # 创建一个由PositionwiseFeedForward模块组成的列表，列表的长度为行为的数量
            self.pff = nn.ModuleList(
                [PositionwiseFeedForward(d_model=d_model, d_ff=d_ff, dropout=dropout) for i in range(n_b)])
        else:
            # 否则，直接创建一个PositionwiseFeedForward模块
    ...
    def multi_behavior_pff(self, x, b_seq):
        ...
        b_seq: B x T, 0 表示填充。
        """
        outputs = [torch.zeros_like(x)]  # 初始化一个包含与x相同大小的全零张量的列表
        for i in range(self.n_b):  # 对于每一个行为
            outputs.append(self.pff[i](x))  # 将经过PositionwiseFeedForward模块处理的结果添加到列表中
        # 使用einsum函数按照指定的公式将输出张量的列表进行加权和
    ...
    def forward(self, x, b_seq=None):
        if self.bpff and self.n_b > 1:  # 如果使用行为特定的PFF并且行为数量大于1
            return self.multi_behavior_pff(x, b_seq)  # 调用multi_behavior_pff方法处理输入
        else:
            return self.pff(x)  # 否则，直接调用PositionwiseFeedForward模块处理输入
